...
class Service {
      ...
      static async allServices (perPage, offset, userId) {
         ...
         const services = await pool.execute('SELECT s.id, s.user_id, s.title, s.description, s.cost, s.duration, s.category, s.created_at, u.email, u.services_rendered FROM service as s JOIN user as u ON u.id = s.user_id WHERE user_id != ? ORDER BY s.created_at DESC LIMIT ? OFFSET ?', [userId, String(perPage), String(offset)]);
